...
function UsruarioSeleccionado(cui){
    alert(cui)
    let usuarios = docentesCargados.find(alumno => alumno.CUI == cui);
    console.log(usuarios);
    LlenarSelectCursos(usuarios.idCurso)
    LlenarSelectHorariosPorCurso(usuarios.idCurso, usuarios.idHorario)
    LlenarSelectSeccionesPorCursoYHorario(usuarios.idCurso, usuarios.idHorario, usuarios.idSeccion)
}
...
async function ObtenerDocentes() {
    ...
    return fetch(asignacionAlumnosPath, {
        method: 'POST',
...
function asignacionMasiva() {
    ...
    data.tkn = sessionStorage.getItem('token');
    data.metodo = 4;
    data.MaxAsignacionesPorSeccion =  document.querySelector('input[name="MaxAsignacionesPorSeccion"]').value 
