...
const lblMantenimiento = 'Tipo de Escala'; 

export async function TipoEscala(mantenimientoBody) {
...
async function obtenerMantenimiento() {
    ...
    try {
        ...
        const result = await request.execute('sp_consultarTipoEscala');
 ...
 async function nuevoMantenimiento(mantenimientoBody) {
    ...
    try {
        ...
        const Result = await request.execute('sp_insertarTipoEscala');
...
async function actualizarMantenimiento(mantenimientoBody) {
    ...
    try {
        ...
        const pool = await poolPromise;
        const request = pool.request();
        request.input('idTipoEscala', sql.Int, Mantenimiento.idTipoEscala);
        request.input('descripcion', sql.NVARCHAR(500), Mantenimiento.descripcion);
        request.output('resultado', sql.Int);
        
        const Result = await request.execute('sp_actualizarTipoEscala');
        const resultado = Result.output.resultado;
...
async function eliminarMantenimiento(mantenimientoBody) {
    ...
    try {
        ...
        const Result = await request.execute('sp_eliminarTipoEscala');
        const resultado = Result.output.resultado;
