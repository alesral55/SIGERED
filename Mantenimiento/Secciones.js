...
const lblMantenimiento = 'Seccion'; 

export async function Secciones(mantenimientoBody) {
...
async function obtenerMantenimiento() {
    ...
    try {
        ...
        const result = await request.execute('sp_consultarSecciones');
 ...
 async function nuevoMantenimiento(mantenimientoBody) {
    ...
    try {
        ...
        request.input('descripcion', sql.NVARCHAR(200), Mantenimiento.descripcion);
        request.input('idHorario', sql.Int, Mantenimiento.idHorario);
        request.output('resultado', sql.Int);
        
        const Result = await request.execute('sp_insertarSecciones');
...
async function actualizarMantenimiento(mantenimientoBody) {
    ...
    try {
        ...
        const pool = await poolPromise;
        const request = pool.request();
        request.input('idSeccion', sql.Int, Mantenimiento.idSeccion);
        request.input('idCurso', sql.Int, Mantenimiento.idCurso);
        request.input('descripcion', sql.NVARCHAR(200), Mantenimiento.descripcion);
        request.input('idHorario', sql.Int, Mantenimiento.idHorario);
        request.output('resultado', sql.Int);
        
        const Result = await request.execute('sp_actualizarSecciones');
        const resultado = Result.output.resultado;
...
async function eliminarMantenimiento(mantenimientoBody) {
    ...
    try {
        ...
        const Result = await request.execute('sp_eliminarSecciones');
        const resultado = Result.output.resultado;
