...
const lblMantenimiento = 'Grupo Etnico'; 

export async function GrupoEtnico(mantenimientoBody) {
...
async function obtenerMantenimiento() {
    ...
    try {
        ...
        const result = await request.execute('sp_consultarGrupoEtnico');
 ...
 async function nuevoMantenimiento(mantenimientoBody) {
    ...
    try {
        ...
        const request = pool.request();
        request.input('nombreGrupo', sql.NVARCHAR(500), Mantenimiento.nombreGrupo);
        request.input('descripcion', sql.NVARCHAR(1000), Mantenimiento.descripcion);
        request.output('resultado', sql.Int);
        
        const Result = await request.execute('sp_insertarGrupoEtnico');
...
async function actualizarMantenimiento(mantenimientoBody) {
    ...
    try {
        ...
        const pool = await poolPromise;
        const request = pool.request();
        request.input('idGrupo', sql.Int, Mantenimiento.idGrupo);
        request.input('nombreGrupo', sql.NVARCHAR(500), Mantenimiento.nombreGrupo);
        request.input('descripcion', sql.NVARCHAR(1000), Mantenimiento.descripcion);
        request.output('resultado', sql.Int);
        
        const Result = await request.execute('sp_actualizarGrupoEtnico');
        const resultado = Result.output.resultado;
...
async function eliminarMantenimiento(mantenimientoBody) {
    ...
    try {
        ...
        const Result = await request.execute('sp_eliminarGrupoEtnico');
        const resultado = Result.output.resultado;
