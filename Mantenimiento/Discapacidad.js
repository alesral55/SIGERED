...
const lblMantenimiento = 'Discapacidad'; 

export async function Discapacidad(mantenimientoBody) {
...
async function obtenerMantenimiento() {
    ...
    try {
        ...
        const result = await request.execute('sp_consultarDiscapacidades');
 ...
 async function nuevoMantenimiento(mantenimientoBody) {
    ...
    try {
        ...
        const Result = await request.execute('sp_insertarDiscapacidades');
...
async function actualizarMantenimiento(mantenimientoBody) {
    ...
    try {
        ...
        const pool = await poolPromise;
        const request = pool.request();
        request.input('idDiscapacidad', sql.Int, Mantenimiento.idDiscapacidad);
        request.input('nombre', sql.NVARCHAR(500), Mantenimiento.nombre);
        request.input('descripcion', sql.NVARCHAR(1000), Mantenimiento.descripcion);
        request.output('resultado', sql.Int);
        
        const Result = await request.execute('sp_actualizarDiscapacidades');
        const resultado = Result.output.resultado;
...
async function eliminarMantenimiento(mantenimientoBody) {
    ...
    try {
        ...
        const Result = await request.execute('sp_eliminarDiscapacidades');
        const resultado = Result.output.resultado;
